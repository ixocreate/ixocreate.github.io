...
var config = {
  ...
  src_path: 'assets', //change to your source path
  public_path: '../ixocreate_theme', //change to your target path
...
config.gulpNotify = function(message) {
  return {
    ...
    icon: null,
    timeout: 2,
    contentImage: './gulp-tasks/notify-icon.png'
  };
};

var getTask = function(task, extra) {
  return require('./gulp-tasks/' + task)(gulp, plugins, config, extra);
};
...
gulp.task('default', gulp.parallel('symlink', 'sass-vendor', 'sass', 'js-vendor', 'js'));
